...
function webRtcPlayer(parOptions) {
  ...
  setupDataChannelCallbacks = function (datachannel) {
    try {
      ...
      datachannel.addEventListener("open", (e) => {
        console.log(
          `Data channel connected: ${datachannel.label}(${datachannel.id})`
        );
      ...
      datachannel.addEventListener("close", (e) => {
        console.log(
          `Data channel disconnected: ${datachannel.label}(${datachannel.id}`,
          e
        );
      });
      ...
      datachannel.addEventListener("error", (e) => {
        console.error(
          `Data channel error: ${datachannel.label}(${datachannel.id}`,
          e
        );
      });
      ...
      stats.forEach((stat) => {
        ...
        if (
          ...
          if (self.aggregatedStats && self.aggregatedStats.timestamp) {
            ...
            if (
              stat.codecId &&
              self.aggregatedStats.codecs &&
              self.aggregatedStats.codecs.hasOwnProperty(stat.codecId)
            ) {
        ...
        if (
          ...
          (stat.mediaType === "audio" ||
            stat.id.toLowerCase().includes("audio"))
        ) {
          ...
          if (self.aggregatedStats && self.aggregatedStats.timestamp) {
            ...
            if (self.aggregatedStats.audioBytesReceived) {
              newStat.audioBitrate =
                (8 *
                  (newStat.audioBytesReceived -
                    self.aggregatedStats.audioBytesReceived)) /
                (stat.timestamp - self.aggregatedStats.timestamp);
  ...
  this.createOffer = function () {
    ...
    setupTransceiversAsync(self.pcClient).finally(function () {
      self.dcClient = createDataChannel(
        self.pcClient,
        "cirrus",
  ...
  this.receiveOffer = function (offer) {
    ...
    if (!self.pcClient) {
      ...
      self.pcClient.setRemoteDescription(offer).then(() => {
        setupTransceiversAsync(self.pcClient).finally(function () {
          self.pcClient
            .createAnswer()
            .then((answer) => {
  ...
  this.receiveSFUPeerDataChannelRequest = function (channelData) {
    ...
    const sendDataChannel = self.pcClient.createDataChannel(
    ...
    if (unidirectional) {
      ...
      const recvDataChannel = self.pcClient.createDataChannel(
        "recv-datachannel",
        recvOptions
      );
