let unquantizeAndDenormalizeUnsigned, receivedBytesMeasurement, receivedBytes;
...
function setupWebRtcPlayer(htmlElement, config) {
  ...
  webRtcPlayerObj.onDataChannelMessage = function (data) {
    ...
    if (view[0] === ToClientMessageType.QualityControlOwnership) {
      ...
      console.error(
        `Custom data channel message with message type that is unknown to the Pixel Streaming protocol. Does your ToClientMessageType enum need updating? The message type was: ${view[0]}`
      );
    }
  };
...
const ControlSchemeType = {
  ...
  // 鼠标可以悬停在WebRTC播放器上，因此用户需要单击并拖动来控制摄像头的方向。
  // 修改
  HoveringMouse: 0,
};
...
function connect() {
  ...
  // ws = new WebSocket(connectionUrl + "ws");
