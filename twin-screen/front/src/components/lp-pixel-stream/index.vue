...
function onWebRtcAnswer(webRTCData) {
  ...
  webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
    ...
    if (statsDiv) statsDiv.innerHTML = statsText;
...
const heartbeat = () => {
  ...
  heartbeat_timer = setTimeout(() => {
    ...
    // console.log(JSON.stringify({ type: 'ping', time: new Date().getTime() }));
    heartbeat();
  }, heartbeat_interval);
};
...
<template>
  <div id="playerUI" class="pixel-stream">
    <div id="player" class="player"></div>
    <div id="overlay" class="overlay text-light bg-dark">
      <div>
        <div id="qualityStatus" class="greyStatus">status</div>
...
.overlay button:hover {
  color: #ffffff;
  background: #1ea6fc;
  box-shadow: 0px 3px 16px 0px rgba(31, 166, 250, 0.52);
}
