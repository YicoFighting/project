      ...
      <el-tabs v-model="activeName" class="oppo-tabs" @tab-click="handleClick">
        <template v-for="category in data.data">
          <el-tab-pane :label="category.title" :name="category.title">
            <grid-view :category="category.productDetailss"></grid-view>
          </el-tab-pane>
        </template>
      </el-tabs>
...
import { getDetailInfo, type IDetailType } from "@/service/detail";
...
const { data } = await getDetailInfo(type as IDetailType);
